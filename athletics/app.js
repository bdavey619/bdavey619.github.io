// Athletics Morning Brief – frontend renderer
...
function buildSubhead(b) {
  ...
  if (streakMatch && parseInt(streakMatch[1]) >= 2) {
    parts.push(`Athletics extend win streak to ${streakMatch[1]}`);
  } else if (lg.result === 'W') {
    parts.push('Athletics win');
  } else {
    parts.push('Athletics fall');
  }
...
function renderLastGame(lg) {
  ...
  if (lg.status !== 'final') {
    body.innerHTML = `<p class="off-day">No game yesterday. Athletics were off.</p>`;
    show('last-game');
    return;
  }
  ...
  const gameContext = lg.context_line || (() => {
    const venue = lg.home ? 'Sutter Health Park' : null;
...
function renderLinescore(lg) {
  ...
  const topRow    = lg.home ? buildRow(lg.opponent, oppRow, false) : buildRow('ATH', teamRow, true);
  const bottomRow = lg.home ? buildRow('ATH', teamRow, true)        : buildRow(lg.opponent, oppRow, false);
  return `
    <div class="linescore">
      <table>
...
function renderAhead(ng, standings) {
  ...
  if (standings?.length) {
    html += `<table class="standings-table">
      <thead><tr><th class="team">AL West</th><th>W</th><th>L</th><th>GB</th><th>L10</th></tr></thead>
      <tbody>`;
    for (const row of standings) {
      const isAthletics = /athletics|^ath$/i.test(row.team);
      html += `<tr class="${isAthletics ? 'athletics' : ''}">
    ...
    // Division context: compare Athletics vs leader last-10
    const athRow = standings.find(r => /athletics|^ath$/i.test(r.team));
    const leaderRow = standings[0];
    if (athRow?.last10 && leaderRow?.last10 && leaderRow !== athRow) {
      html += `<p class="division-context">Athletics ${athRow.last10} in last 10, ${leaderRow.team} ${leaderRow.last10}.</p>`;
    }
  }
