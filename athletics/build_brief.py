"""
build_brief.py — Athletics Morning Brief data fetcher

Pulls Athletics data from the MLB Stats API and writes brief.json
...
from engine.team_config import ATHLETICS  # noqa: E402
...
def _fetch_schedule(start, end):
    """Fetch Athletics schedule between two dates (inclusive)."""
    data = get(
        "schedule",
        sportId=1,
        teamId=CFG.team_id,
...
def _extract_key_performers(box, sd_side):
    """Top 2 hitters by hits/HR. Key pitcher = actual starter."""
...
def _extract_full_box_score(box, sd_side):
    """Full batting + pitching table for the Athletics in this game."""
    sd_team = box["teams"][sd_side]
    players = sd_team.get("players", {})
    pitcher_ids = sd_team.get("pitchers", [])
...
def get_insight(team, last_game=None):
    ...
    pitching_strong = era_val < 3.80
    offense_weak    = ops_val < 0.700
...
_NOVELTY_PENALTY_YESTERDAY = 0.10
_NOVELTY_PENALTY_RECENT    = 0.05
_DISTINCTIVENESS_PENALTY_DIRECT = 0.10
_MIN_HOOK_SCORE = 0.45

_HOOK_TYPE_THEMES = {
    "race_context":         "race",
    "team_momentum":        "momentum",
    "pitcher_form":         "pitching",
    "opponent_weakness":    None,
    "opponent_cold_streak": None,
}
...
def _detect_page_themes(subhead, insight):
    ...
    if any(kw in text for kw in [
        "back in the", "al west race", "division race", "games back",
        "back in the west", "back in the al west",
    ]):
        themes.add("race")
...
def build():
    ...
    save_story_state(story_state, STORY_STATE_PATH)
    if narrative_copy:
