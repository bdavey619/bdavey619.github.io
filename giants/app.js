...
function renderLastGame(lg) {
  ...
  const gameContext = lg.context_line || (() => {
    const venue = lg.home ? 'Oracle Park' : null;
...
function renderLinescore(lg) {
  ...
  const topRow    = lg.home ? buildRow(lg.opponent, oppRow, false) : buildRow('SF', teamRow, true);
  const bottomRow = lg.home ? buildRow('SF', teamRow, true)        : buildRow(lg.opponent, oppRow, false);
  return `
    <div class="linescore">
      <table>
...
function renderAhead(ng, standings) {
  ...
  if (standings?.length) {
    ...
    for (const row of standings) {
      const isGiants = /giants|^sf$/i.test(row.team);
      html += `<tr class="${isGiants ? 'giants' : ''}">
    ...
    // Division context: compare Giants vs leader last-10
    const giantsRow = standings.find(r => /giants|^sf$/i.test(r.team));
    const leaderRow = standings[0];
    if (giantsRow?.last10 && leaderRow?.last10 && leaderRow !== giantsRow) {
      html += `<p class="division-context">Giants ${giantsRow.last10} in last 10, ${leaderRow.team} ${leaderRow.last10}.</p>`;
    }
  }
