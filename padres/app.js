...
function renderSummaryBar(b) {
  ...
  if (lg.status === 'final' && lg.score) {
    const vs = lg.home ? 'vs' : '@';
    $('sb-last').textContent = `${lg.result} ${lg.score.sd}-${lg.score.opp}`;
  } else {
    $('sb-last').textContent = '—';
  }
...
function renderLastGame(lg) {
  ...
  let html = `
    <div class="lg-headline">
      <span class="lg-score">${lg.score.sd}–${lg.score.opp}</span>
...
function renderLinescore(lg) {
  ...
  const buildRow = (name, row, isPadres) => {
    ...
    cells += `<td class="total">${isPadres ? lg.score.sd : lg.score.opp}</td>`;
    return `<tr>${cells}</tr>`;
  };
...
function renderAhead(ng, standings) {
  ...
  if (ng) {
    ...
    if (ng.probable && (ng.probable.sd || ng.probable.opp)) {
      const sd = ng.probable.sd || 'TBD';
